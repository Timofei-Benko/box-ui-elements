...
function (_React$Component) {
  ...
  function Logger(props) {
    ...
    _defineProperty(_assertThisInitialized(_this), "handleDataReadyMetric", function (data, uniqueId) {
      if (!isMarkSupported) {
        return;
      }

      _this.logUniqueMetric(METRIC_TYPE_ELEMENTS_LOAD_METRIC, EVENT_DATA_READY, data, uniqueId);
    });
    ...
    _this.loggerProps = {
      onDataReadyMetric: _this.handleDataReadyMetric,
  ...
  _createClass(Logger, [{
     ...
     */
    value: function createEventName(name, uniqueId) {
      var source = this.props.source;
      var eventName = "".concat(source, "::").concat(name);
      return uniqueId ? "".concat(eventName, "::").concat(uniqueId) : eventName;
    }
    /**
    ...
    key: "logUniqueMetric",
    value: function logUniqueMetric(type, name, data, uniqueId) {
      var eventName = this.createEventName(name, uniqueId);
