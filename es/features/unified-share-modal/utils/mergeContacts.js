...
var mergeContacts = function mergeContacts(existingContacts, fetchedContacts) {
  var contactsMap = Object.keys(fetchedContacts).reduce(function (map, email) {
    var contact = fetchedContacts[email]; // Since objects are case-sensitive, normalize the key to lowercase.

    map[email.toLowerCase()] = _objectSpread({}, contact, {
      text: contact.name,
  ...
  return existingContacts.map(function (contact) {
    ...
    return (// Normalize the getter in contactsMap so that matching existing contacts will be case-insensitive
