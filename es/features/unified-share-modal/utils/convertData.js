...
export var convertCollabsRequest = function convertCollabsRequest(collabRequest, existingCollaboratorsList) {
  ...
  var collabSet = new Set();

  if (existingCollaboratorsList) {
    existingCollaboratorsList.collaborators.forEach(function (collab) {
  ...
  var groups = groupIDArray.filter(function (groupID) {
    return !collabSet.has(groupID);
  ...
  var users = emailArray.filter(function (email) {
    return !collabSet.has(email);
  }).map(function (email) {
    return _objectSpread({
      accessible_by: {
        login: email,
