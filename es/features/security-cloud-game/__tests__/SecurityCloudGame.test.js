...
describe('features/security-cloud-game/SecurityCloudGame', function () {
  ...
  var DragCloudMock = function DragCloudMock() {
    return React.createElement("div", null);
  };

  var DropCloudMock = function DropCloudMock() {
    return React.createElement("div", null);
  };
  ...
  test('should correctly render', function () {
    var mock = sandbox.mock(SecurityCloudGame.prototype);
    mock.expects('renderMessage').once();
    mock.expects('setGameBoardHeight').once();
    var component = mount(React.createElement(SecurityCloudGame, {
      cloudSize: 50,
      height: 100,
      width: 100
    }));
  ...
  test('should correctly calculate cloud positions', function () {
    var spy = sandbox.spy(SecurityCloudGame.prototype, 'getRandomCloudPosition');
    var stubRandomMethod = sandbox.stub(SecurityCloudGame.prototype, 'getRandom').returns(0.1);
  ...
  test('should retry getRandomCloudPosition if overlap was found', function () {
    var spy = sandbox.spy(SecurityCloudGame.prototype, 'getRandomCloudPosition');
    var stubMethod = sandbox.stub(SecurityCloudGame.prototype, 'checkOverlap');
    stubMethod.onFirstCall().returns(true);
    stubMethod.onSecondCall().returns(false);
    mount(React.createElement(SecurityCloudGame, {
      cloudSize: 50,
      height: 100,
      width: 100
  ...
  test('should return true if checkOverlap is called with an overlapping position', function () {
    var mock = sandbox.mock(SecurityCloudGame.prototype);
    mock.expects('checkOverlap').once().returns(false);
    var component = mount(React.createElement(SecurityCloudGame, {
      cloudSize: 10,
      height: 100,
      width: 100
  ...
  test('should render drop region and clouds', function () {
    var stubMethod = sandbox.stub(SecurityCloudGame.prototype, 'getRandomCloudPosition').returns({
      x: 5,
      y: 5
    });
    sandbox.stub(SecurityCloudGame.prototype, 'checkOverlap').returns(false);
    stubMethod.onFirstCall().returns({
      x: 10,
      y: 10
    });
    stubMethod.onSecondCall().returns({
      x: 5,
      y: 5
    });
    var component = mount(React.createElement(SecurityCloudGame, {
  ...
  test('should only render drop cloud if isValidDrop is false', function () {
    var stubMethod = sandbox.stub(SecurityCloudGame.prototype, 'getRandomCloudPosition');
    sandbox.stub(SecurityCloudGame.prototype, 'checkOverlap').returns(false);
    stubMethod.onFirstCall().returns({
      x: 10,
      y: 10
    }); // this one is the drop position

    stubMethod.onSecondCall().returns({
      x: 5,
      y: 5
    });
    var component = shallow(React.createElement(SecurityCloudGame, {
      cloudSize: 10,
      height: 100,
      width: 100
    }));
  ...
  test('should render an instructional message when renderMessage is called', function () {
    ...
    var instance = component.instance();
    var message = instance.renderMessage();
    expect(message.props.defaultMessage).toEqual('For security purposes, please drag the white cloud into the dark cloud.');
  });
  test('should render a success message when renderMessage is called and isValidDrop is true', function () {
    var mock = sandbox.mock(SecurityCloudGame.prototype);
    mock.expects('checkOverlap').once().returns(false);
    var component = mount(React.createElement(SecurityCloudGame, {
      cloudSize: 10,
  ...
  test('should correctly set isOverlap onDrag', function () {
    var stubCheckMethod = sandbox.stub(SecurityCloudGame.prototype, 'checkOverlap').returns(false);
    var component = mount(React.createElement(SecurityCloudGame, {
      cloudSize: 10,
      height: 100,
      width: 100
    ...
    expect(instance.state.isOverlap).toBe(true); // onDrag is throttled so need to be wait a tick.
  ...
  test('should set isValidDrop on valid drop', function () {
    var onDrop = sinon.spy();
    var component = mount(React.createElement(SecurityCloudGame, {
      cloudSize: 10,
      height: 100,
      width: 100,
