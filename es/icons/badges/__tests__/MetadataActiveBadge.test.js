...
import MetadataActiveBadge from '../MetadataActiveBadge';
describe('icons/badges/MetadataActiveBadge', function () {
  var getWrapper = function getWrapper() {
    var props = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    return shallow(React.createElement(MetadataActiveBadge, props));
  };

  test('should correctly render default icon', function () {
    var wrapper = getWrapper();
    expect(wrapper.is('AccessibleSVG')).toBe(true);
    expect(wrapper.hasClass('metadata-active-badge')).toBe(true);
