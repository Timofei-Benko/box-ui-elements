...
describe('util/sorter', function () {
  ...
  test('should not sort when already sorted', function () {
    item.item_collection.entries = ['fo1', 'fo2', 'fo3', 'f1', 'f2', 'f3', 'w1', 'w2', 'w3'];
    item.item_collection.order = [{
      by: 'name',
      direction: SORT_ASC
    }];
    var sorted = sort(item, 'name', SORT_ASC, cache);
    expect(sorted.item_collection.entries).toEqual(['fo1', 'fo2', 'fo3', 'f1', 'f2', 'f3', 'w1', 'w2', 'w3']);
  });
  test('should sort with name desc', function () {
    item.item_collection.entries = ['fo1', 'fo2', 'fo3', 'f1', 'f2', 'f3', 'w1', 'w2', 'w3'];
    item.item_collection.order = [{
      by: 'name',
      direction: SORT_ASC
    }];
    var sorted = sort(item, 'name', SORT_DESC, cache);
    expect(sorted.item_collection.entries).toEqual(['fo3', 'fo2', 'fo1', 'f3', 'f2', 'f1', 'w3', 'w2', 'w1']);
  });
  test('should sort with date desc', function () {
    item.item_collection.entries = ['fo3', 'f1', 'f2', 'w1', 'w2', 'fo1', 'fo2', 'f3', 'w3'];
    item.item_collection.order = [{
      by: 'name',
      direction: SORT_ASC
    }];
    var sorted = sort(item, 'modified_at', SORT_DESC, cache);
    expect(sorted.item_collection.entries).toEqual(['fo3', 'fo2', 'fo1', 'f3', 'f2', 'f1', 'w3', 'w2', 'w1']);
  });
  test('should sort with date asc', function () {
    item.item_collection.entries = ['fo1', 'fo4', 'f1', 'f3', 'w2', 'w1', 'fo2', 'fo3', 'f2', 'w3'];
    item.item_collection.order = [{
      by: 'name',
      direction: SORT_ASC
    }];
    var sorted = sort(item, 'modified_at', SORT_ASC, cache);
    expect(sorted.item_collection.entries).toEqual(['fo1', 'fo4', 'fo2', 'fo3', 'f1', 'f2', 'f3', 'w1', 'w2', 'w3']);
  });
  test('should sort with interacted date desc', function () {
    item.item_collection.entries = ['fo3', 'f1', 'f2', 'w1', 'w2', 'fo1', 'fo2', 'f3', 'w3'];
    item.item_collection.order = [{
      by: 'name',
      direction: SORT_ASC
    }];
    var sorted = sort(item, 'interacted_at', SORT_DESC, cache);
    expect(sorted.item_collection.entries).toEqual(['fo2', 'fo3', 'fo1', 'f2', 'f3', 'f1', 'w1', 'w2', 'w3']);
  });
  test('should sort with interacted date asc', function () {
    item.item_collection.entries = ['fo1', 'fo4', 'f1', 'f3', 'w2', 'w1', 'fo2', 'fo3', 'f2', 'w3', 'w4'];
    item.item_collection.order = [{
      by: 'name',
      direction: SORT_ASC
    }];
    var sorted = sort(item, 'interacted_at', SORT_ASC, cache);
    expect(sorted.item_collection.entries).toEqual(['fo1', 'fo4', 'fo3', 'fo2', 'f1', 'f3', 'f2', 'w3', 'w2', 'w4', 'w1']);
  });
  test('should sort with interacted date desc', function () {
    item.item_collection.entries = ['fo3', 'f1', 'f2', 'w1', 'w2', 'fo1', 'fo2', 'f3', 'w3'];
    item.item_collection.order = [{
    ...
    var sorted = sort(item, 'size', SORT_DESC, cache);
    expect(sorted.item_collection.entries).toEqual(['fo2', 'fo1', 'fo3', 'f1', 'f3', 'f2', 'w3', 'w1', 'w2']);
  });
  test('should sort with size asc', function () {
    item.item_collection.entries = ['fo1', 'fo4', 'f1', 'f3', 'w2', 'w1', 'fo2', 'fo3', 'f2', 'w3', 'w4'];
    item.item_collection.order = [{
      by: 'name',
      direction: SORT_ASC
    }];
    var sorted = sort(item, 'size', SORT_ASC, cache);
    expect(sorted.item_collection.entries).toEqual(['fo3', 'fo1', 'fo2', 'fo4', 'f2', 'f3', 'f1', 'w2', 'w1', 'w3', 'w4']);
  });
  test('should sort with default type file and modified date when interacted date and type is missing', function () {
  ...
  test('should sort with default size 0 when size is missing', function () {
    item.item_collection.entries = ['fo5', 'fo6', 'f4', 'f5'];
    item.item_collection.order = [{
      by: 'name',
      direction: SORT_ASC
  ...
  test('should sort with default name when name is missing', function () {
  ...
  test('should sort with name asc', function () {
    ...
    var sorted = sort(item, 'name', SORT_ASC, cache);
    expect(sorted.item_collection.entries).toEqual(['fo1', 'fo4', 'fo2', 'fo3', 'f1', 'f2', 'f3', 'w1', 'w4', 'w2', 'w3']);
  });
  test('should throw with a bad sortBy', function () {
    item.item_collection.entries = ['w1', 'w3', 'fo1', 'fo4', 'f1', 'w2', 'w4', 'f3', 'f2', 'fo2', 'fo3'];
    item.item_collection.order = [{
      by: 'name',
      direction: SORT_DESC
  ...
  test('should throw with a bad item when no item_collection', function () {
    item.item_collection = null;
    expect(sort.bind(sort, item, 'name', SORT_ASC, cache)).toThrow(Error, /Bad box item/);
  });
  test('should throw with a bad item when no entries', function () {
  ...
  describe('sortFeedItems()', function () {
    ...
    test('should sort items based on date', function () {
      var sorted = sortFeedItems(comments, tasks, annotations);
